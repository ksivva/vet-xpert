...
const AnimalCard: React.FC<AnimalCardProps> = ({ animal, onClick }) => {
  return (
    <div 
      className="card-container animate-fade-in cursor-pointer hover:shadow-lg transition-shadow"
      onClick={onClick}
    >
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="text-xl font-bold">{animal.visualTag}</h3>
          <p className="text-gray-500">{animal.gender}</p>
        </div>
        <span className="bg-vetxpert-purple text-white px-3 py-1 rounded-full text-sm">
